...
class TransparentWindow(QMainWindow):
    ...
    def process_hand_data(self, hand_data):
        ...
        try:
            # Debug logging
            print(f"Processing hand data: {json.dumps(hand_data, indent=2)}")
            ...
            if "gesture" in hand_data and hand_data["gesture"] != "none":
                self.status_label.setText(f"Detected gesture: {hand_data['gesture']}")
                
        except Exception as e:
            print(f"Error writing hand data: {str(e)}")
            import traceback
            traceback.print_exc()
...
class HandTrackingThread(QThread):
    ...
    def run(self):
        ...
        # Set a more reasonable resolution
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
        
        # Initialize hand data variables
left_fingertips = None
right_fingertips = None
left_fingers = None
right_fingers = None
        ...
        if results.multi_hand_landmarks:
            ...
            hand_data = {
                "left_hand": {"fingertips": left_fingertips} if left_fingertips is not None else {"fingertips": []},
                "right_hand": {"fingertips": right_fingertips} if right_fingertips is not None else {"fingertips": []},
                "gesture": current_gesture
            }
            
            # Debug logging
            print(f"Emitting hand data: {json.dumps(hand_data, indent=2)}")
