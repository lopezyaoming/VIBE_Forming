...
# Determine project root and common paths
BASE_DIR = Path(__file__).resolve().parents[2]
INPUT_OPTIONS_DIR = str(BASE_DIR / "input" / "options")
LIVE_DATA_FILE = str(BASE_DIR / "output" / "live_hand_data.json")
print(f"[VIBE DEBUG] Live JSON file: {LIVE_DATA_FILE}")
...
class IMAGE_PT_reload_all(bpy.types.Operator):
    ...
    def execute(self, context):
        # Directory for option images
        image_dir = INPUT_OPTIONS_DIR
...
# === CONFIGURATION ===
# LIVE_DATA_FILE defined above dynamically
...
RENDER_OUTPUT_DIR = str(BASE_DIR / "input" / "COMFYINPUTS" / "blenderRender")
print(f"[VIBE DEBUG] Render output directory: {RENDER_OUTPUT_DIR}")
...
class REALTIME_OT_update_mesh(bpy.types.Operator):
    ...
    def read_live_data(self):
        print(f"[VIBE DEBUG] Attempting to read live data JSON")
        ...
        try:
            with open(LIVE_DATA_FILE, "r") as f:
                ...
                print(f"[VIBE DEBUG] Raw JSON content: '{content}'")
                if not content:
                    logging.debug("Live data file exists but is empty")
                    return {}
                data = json.loads(content)
                print(f"[VIBE DEBUG] Parsed JSON data: {data}")
                return data
    ...
    def render_multiview(self):
        ...
        try:
            ...
            base_output_dir = str(BASE_DIR / "output" / "generated" / "Models")
