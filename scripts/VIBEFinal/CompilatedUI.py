...
OUTPUT_JSON = os.path.join(OUTPUT_DIR, "live_hand_data.json")
...
LEFT_HAND_COLOR = (235, 255, 5)
RIGHT_HAND_COLOR = (235, 255, 5)
...
class TransparentWindow(QMainWindow):
    def __init__(self):
        ...
        self.hand_tracking_thread.frame_ready.connect(self.update_hand_frame)
        self.hand_tracking_thread.hand_data_ready.connect(self.process_hand_data)
    ...
    def update_hand_frame(self, frame):
        # Convert OpenCV frame to QImage
        height, width, channel = frame.shape
        bytes_per_line = 3 * width
        q_image = QImage(frame.data, width, height, bytes_per_line, QImage.Format_RGB888)
        
        # Update the hand tracking display
        if hasattr(self, 'hand_frame'):
            self.hand_frame.setPixmap(QPixmap.fromImage(q_image))
            
    def process_hand_data(self, hand_data):
        """Process hand data and update UI accordingly"""
        try:
            # Ensure the output directory exists
            os.makedirs(os.path.dirname(OUTPUT_JSON), exist_ok=True)
            
            # Ensure hand data has the correct structure
            if not isinstance(hand_data, dict):
                hand_data = {}
            
            if "left_hand" not in hand_data:
                hand_data["left_hand"] = {"fingertips": []}
            if "right_hand" not in hand_data:
                hand_data["right_hand"] = {"fingertips": []}
            if "gesture" not in hand_data:
                hand_data["gesture"] = "none"
            
            # Update hand data in the JSON file
            with open(OUTPUT_JSON, "w") as f:
                json.dump(hand_data, f, indent=2)
                f.flush()
                os.fsync(f.fileno())
            
            # Update UI if gesture is detected
            if hand_data["gesture"] != "none":
                self.status_label.setText(f"Detected gesture: {hand_data['gesture']}")
            
        except Exception as e:
            self.status_label.setText(f"Error writing hand data: {str(e)}")
    ...
    def init_ui(self):
        ...
        # Add hand tracking frame
        self.hand_frame = QLabel()
        self.hand_frame.setFixedSize(320, 240)
        self.hand_frame.setStyleSheet("""
            QLabel {
                background-color: rgba(20, 20, 20, 150);
                border: none;
                border-radius: 10px;
        ...
        # Add hand frame to layout
        main_layout.addWidget(self.hand_frame, alignment=Qt.AlignRight)
    ...
    def start_hand_tracking(self):
        """Start hand tracking automatically"""
        try:
            # Create hand tracking request file
            hand_tracking_file = os.path.join(BASE_DIR, "hand_tracking_request.txt")
            with open(hand_tracking_file, "w") as f:
                f.write("start")
    ...
    def closeEvent(self, event):
        ...
        try:
            # Create a request to delete the HandTracking collection
            delete_request_file = os.path.join(BASE_DIR, "delete_hand_tracking.txt")
            with open(delete_request_file, "w") as f:
                f.write("delete")
            
            # Stop the hand tracking thread
            if self.hand_tracking_thread.isRunning():
                self.hand_tracking_thread.stop()
                self.hand_tracking_thread.wait(1000)
        except:
            pass
    ...
    def handle_completion(self, success, message):
        ...
        if success:
            self.status_label.setText("Generation complete! Please select an image to continue...")
...
def get_fingertips_from_landmarks(landmarks):
    ...
    fingertips = []
    for finger, idx in FINGERTIP_INDICES.items():
...
def get_finger_positions(landmarks):
    """Get the positions of all finger joints"""
    if not landmarks:
        return None
        
    # MediaPipe hand landmark indices for finger joints
    FINGER_JOINTS = {
        'thumb': [2, 3, 4],      # Thumb has 3 joints
        'index': [5, 6, 7, 8],   # Other fingers have 4 joints
        'middle': [9, 10, 11, 12],
        'ring': [13, 14, 15, 16],
        'pinky': [17, 18, 19, 20]
    }
    ...
    for finger, indices in FINGER_JOINTS.items():
        ...
        finger_positions[finger] = positions
    
    return finger_positions

class HandTrackingThread(QThread):
    frame_ready = pyqtSignal(np.ndarray)
    hand_data_ready = pyqtSignal(dict)
    ...
    def run(self):
        try:
            ...
            while self.running:
                ...
                # Convert to RGB first, then flip
                ...
                # Reset hand data for this frame
                left_fingertips = None
                right_fingertips = None
                left_fingers = None
                right_fingers = None
                    ...
                    for hand_idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                        if results.multi_handedness and len(results.multi_handedness) > hand_idx:
                            ...
                            if hand_label == "Left":
                                ...
                                left_fingers = get_finger_positions(hand_landmarks)
                            else:
                                right_fingertips = get_fingertips_from_landmarks(hand_landmarks)
                                right_fingers = get_finger_positions(hand_landmarks)
                
                # Emit frame
                self.frame_ready.emit(rgb_frame)
                
                # Prepare and emit hand data
                hand_data = {
                    "left_hand": {"fingertips": left_fingertips if left_fingertips else []},
                    "right_hand": {"fingertips": right_fingertips if right_fingertips else []},
                    "gesture": "none"  # Default gesture
                }
                self.hand_data_ready.emit(hand_data)
